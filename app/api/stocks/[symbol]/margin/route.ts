...
import { getMarginTrading, getStockSummary } from "@/lib/providers/finmind";
...
export async function GET(
  ...
  try {
    const margin = await getMarginTrading(symbol, range);
    return NextResponse.json(margin);
  } catch {
    return errorResponse("UPSTREAM_ERROR", "無法取得融資融券資料，請稍後再試");
  }
}
